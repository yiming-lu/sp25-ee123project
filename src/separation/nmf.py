...
"""
改进版：使用监督式 NMF 并集成评估与可视化
增加时域 MSE 和频域 SSIM 评估，并显式指定 data_range
"""
import os
import sys
import numpy as np
...
def supervised_nmf(s1, s2, mix, L):
    ...
    K = 20
    nmf1 = NMF(n_components=K, init='nndsvda', solver='mu',
               beta_loss='kullback-leibler', alpha_H=0.01, max_iter=500)
    W1 = nmf1.fit_transform(p1)
    nmf2 = NMF(n_components=K, init='nndsvda', solver='mu',
               beta_loss='kullback-leibler', alpha_H=0.01, max_iter=500)
    W2 = nmf2.fit_transform(p2)
...
def main():
    ...
    # 5) 附加评估：ΔSNR、MSE、频域 SSIM
    print("\n附加评估指标：")
    D_true1, _ = compute_spectra(s1)
    D_true2, _ = compute_spectra(s2)
    log_true1 = 20 * np.log10(np.abs(D_true1) + 1e-6)
    log_true2 = 20 * np.log10(np.abs(D_true2) + 1e-6)
    for idx, (est, orig, log_true) in enumerate(zip(S_est, [s1, s2], [log_true1, log_true2]), 1):
        # ΔSNR & MSE
        noise_in = mix - orig
        noise_out = est - orig
        psig = np.mean(orig**2)
        p_in = np.mean(noise_in**2)
        p_out = np.mean(noise_out**2)
        snr_in = 10*np.log10(psig/(p_in+1e-8))
        snr_out = 10*np.log10(psig/(p_out+1e-8))
        delta_snr = snr_out - snr_in
        mse = np.mean((est - orig)**2)
        # 频域 SSIM（指定 data_range）
        D_est, _ = compute_spectra(est)
        log_est = 20 * np.log10(np.abs(D_est) + 1e-6)
        data_range = log_true.max() - log_true.min()
        ssim_val = ssim(log_true, log_est, data_range=data_range)
        print(f"  源 {idx}: ΔSNR={delta_snr:.2f} dB, MSE={mse:.6f}, SSIM={ssim_val:.4f}")
